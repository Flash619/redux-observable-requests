...
import {
  generateAjaxRequest,
  generateRequestAbortAction,
  generateRequestResponseAction,
  isRequest
} from "./utilities";
import { of } from "rxjs";
...
export const generateRequestSubscriber = (
  options?: RequestSubscriberOptions
) => {
  ...
  const requestSubscriber: Epic<any> = action$ =>
    action$.pipe(
